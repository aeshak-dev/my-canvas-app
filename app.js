...
function updateBrush() {
  if (currentMode === 'draw') {
    ...
    if (fabric.EraserBrush) {
      ...
      // Fallback: set brush color directly to the canvas background color
      canvas.freeDrawingBrush = new fabric.PencilBrush(canvas);
...
// --- UNDO / REDO SYSTEM (REWRITTEN & TESTED) ---
let historyStack = [];
let redoStack = [];
...
function saveState() {
  ...
  // Include custom/overlay properties in serialization
...
function loadCanvasState(jsonString) {
  ...
  canvas.loadFromJSON(jsonString, () => {
    // Re-apply tool states to objects after load
...
document.getElementById('undoBtn').addEventListener('click', () => {
  if (historyStack.length > 1) {
    redoStack.push(historyStack.pop()); // Pop current state
